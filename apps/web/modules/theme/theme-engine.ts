...
export function generatePalette(seed: string, prefix: "primary" | "accent") {
  ...
  const scales = [
    0.96, 0.92, 0.86, 0.78, 0.7, 0.62, 0.54, 0.46, 0.36, 0.26, 0.18,
  ]

  return PALETTE_STOPS.reduce<Record<string, string>>(
    (accumulator, stop, index) => {
      const lightness = Math.max(0.1, Math.min(0.98, scales[index] ?? l))
      accumulator[`--${prefix}-${stop}`] =
        `oklch(${lightness.toFixed(3)} ${Math.max(0.01, c).toFixed(3)} ${h})`
      return accumulator
    },
    {}
  )
